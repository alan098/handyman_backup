            ...
            <div class="row">
                ...
                <div class="col-md-2">
                    @if(!empty( $compra->numero_factura ))
                        <span><p>Factura: <b>{{ $compra->numero_factura }}</b></p></span>
                    @else
                        <span><p>Boleta: <b>{{ $compra->numero_boleta }}</b></p></span>
                    @endif
                </div>
                <div class="col-md-2">
                        ...
                        <tbody>
                            ...
                            @foreach ($detalles as $det)
                                ...
                                <tr>
                                    ...
                                    <td>{{ $det->cod_articulo }}</td>
                                    <td>{{ $det->name }}</td>
                                    <td style="text-align: right">{{ number_format($det->precio_unitario, 0, '', '.') }}</td>
                                    <td style="text-align: right">{{ number_format($det->excenta, 0, '', '.') }}</td>
                                    <td style="text-align: right">{{ number_format($det->gravada_5, 0, '', '.') }}</td>
                                    <td style="text-align: right">{{ number_format($det->gravada_10, 0, '', '.') }}</td>
