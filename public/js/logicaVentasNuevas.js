...
function facCon(lugar){
    if($(lugar).prop('checked')){
        $('#factura_contacto').css('display','block')
        $("#form-cliente #cliente_id" ).trigger('change')
        $('#es_contacto').val('si')
    }else{
        $('#factura_contacto').css('display','none')
        $('#es_contacto').val('no')
    }
}
function llenarContacto(cliente){
    var ruta=url_cliente+"?cliente_id="+cliente;
    getData(ruta).then(function(rta){
        $('#cliente_contacto option').remove().promise().done(function(){
            if (rta['per_de'] != null) {
                $("#form-cliente #persona_name" ).val(rta['per_de'].name);
                $("#form-cliente #persona_direcion" ).val(rta['per_de'].direccion);
                $("#form-cliente #persona_telefono" ).val(rta['per_de'].telefono);
            }
            
            $.each(rta['per'], function(i, item) {
                $("#cliente_contacto").append("<option value="+item.cliente_id+"> "+item.ruc+"-"+item.name+"</option>");  
            });
        });
...
$(document).on("change", "#timbrado_id", function(){
    ...
    timbrados_js.map(function(x) {
        if ($('#timbrado_id').val() == x.timbrado_id) {
            ...
            var fac_table =-'-'+ajustarFac(6,fac_actual);
            var fac_tim_table='Fecha Inicio Vigencia '+x.fecha_ini_vigencia+' - '+x.fecha_fin_vigencia+' Fecha Fin Vigencia ';
            //esto es para la seccion de ver no borrar
            $('#factura_table').text(fac_table);
            $('#timbrado_table').text(x.numero_timbrado);
            $('#vigencia').text(fac_tim_table)
        }
    });
...
$("#add_detalles_venta").click(function() {
    ...
    articulos_js.map(function(x) {
        ...
        }else if((x.tipo == 'insumo' )&& (x.es_vendible== true)){
            optionsP+="<option value='"+x.id+"'> "+x.name+" </option>";
...
function CalculosDetalles(){
    ...
    if ($('#formResumen #seccion_descuento').css('display') == 'block') {//descuento
        ...
        $(".fila_cero").find("input[id='met_monto']").val(puntear(descuento));
        $("#formResumen #iva-total" ).text((iva5(T5) + iva10(T10) ));
    }else{
        $("#formResumen #total-pagar" ).text(total)
        $(".fila_cero").find("input[id='met_monto']").val(puntear(total));
        $("#formResumen #iva-total" ).text((iva5(T5) + iva10(T10) ));
    }
    calcularImporte()
...
const ItemServicios = ({users, num_li=azarServis(),fecha,optionsm,servicio,desde,hasta,precio_actual,servicio_id,iva,porcentaje_colaborador=30,por_fi=0,precio_real,imp=0,imd=0,read,tama=6,display='style="display: block"'}) => `
<tr class="servicio_datos">
            <td>
                <input type="hidden"  value="1" servicio="${servicio_id}" id="cantidad_servicio" name="servis[${num_li}][cantidad]">
                <input type="hidden" id="numero_servicio"  name="servis[${num_li}][servicio]" value='${servicio_id}'>
            ...
            <td style="width:5%" >
                <input type="text" class="form-control  siempreCero" value="${por_fi}"  id="afiliado_por" name="servis[${num_li}][afiliado_por]" readOnly>
            </td>
            <td>
                <select class="select2 form-control" style="width: 100%"  id="afiliado_id" name="servis[${num_li}][afiliado_id]">
                <option value="" selected>Sin Afiliado</option>
            ...
            <td style="width:3%" >
                <input type="text" class="form-control text-righ"  id="descuento_ser" value="${imp}%" readOnly>
            </td>
            <td>
...
`;
$(document).on("change", "#formServicios [id^='afiliado_id']", function(){
    var indice= $(this).closest( ".servicio_datos" );
    var par = parametros_js.find( item => item.clave.trim() == 'porcentaje_afiliado');
    console.log($(indice).find('#colaborador_id').val())
    console.log($(indice).find('#numero_servicio').val())

    var por = comisiones_js.find(item => ((item.colaborador_id == $(indice).find('#colaborador_id').val()) && (item.servicio_id == $(indice).find('#numero_servicio').val())));
    var bas =30
    if (por) {
        bas = por.porcentaje;
    }
    var total= bas - (par.valor);
    if ($(this).val()) {
        $(indice).find("input[id='porcentaje_']").val(total)
        $(indice).find("input[id='afiliado_por']").val( (par.valor) )
    }else{
        $(indice).find("input[id='porcentaje_']").val(bas)
        $(indice).find("input[id='afiliado_por']").val(0)
    }
}); 
        ...
        if (x.id == arti) {
            ...
            x.combo_articulos.map(function(y){
                ...
                if (ele) {
                    ...
                    "Uno de los articulos no posee existencia o esta inactivo !"
                }
